#v6.2 with global dimming via readsensor with timer, no LUT: formula
#                 added for api web_interface.py
#                 dimming starts when time_default calls: wcd.readsensor() see towards end of code here
import configparser
...
import wordclock_plugins.time_default.time_dutch16 as time_dutch16
...
class wordclock_display:
    ...
    def __init__(self, config, wci):
        ...
        self.Helder = config.getint('wordclock_display', 'helder')   #7 - 18
        self.C=0.566
        self.D=0.4
        ...
        elif dialect == 'dutch16':
            self.taw = time_dutch16.time_dutch()
    ...
    def setHelderAndShow(self, helder):
        with self.mutex:
            self.Helder = helder
        self.show()
    ...
    def readsensor(self):                      #call once in time_default to start the dimming thread
        threading.Timer(0.4, self.readsensor).start()
...
def BrightViaFormula(self):
    br= max(1,int(pow(readlux(self),self.C)*self.Helder+self.D))
    return(br)
